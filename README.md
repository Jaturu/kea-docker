...
`docker.cloudsmith.io/isc/docker/your-image:latest`
...
Kea images should be provided with configuration files and have proper ports binded to work correctly.
...
Appropriate  ports should be bound to host IP addresses that will be providing DHCP service.

Kea DHCP v4:
- 67/tpc and 67/udp - DHCP ports and Bulk Lease Querry
- 8000/tcp - Control Agent
- 8001/tcp - High Availability communication

Kea DHCP v6:
- 547/tpc and 547/udp - DHCP ports and Bulk Lease Querry
- 8000/tcp - Control Agent
- 8001/tcp - High Availability communication
...
eg.
```shell
-p 10.4.0.1:67:67/udp
```
For v6 address you need to use a square bracket like this:
```shell
-p [2008:db8::1]:8081:8081/udp
```

## Examples

Here are some example commands to run Kea images.

Kea DHCP v4 with all volumes and ports:
```shell
docker run --volume=./config/kea:/etc/kea  \
                --volume=./:/var/lib/kea  \
                -p 10.4.0.1:67:67/tcp \
                -p 10.4.0.1:67:67/udp \
                -p 10.4.0.1:8000:8000 \
                -p 10.4.0.1:8001:8001 \
                docker.cloudsmith.io/isc/docker/kea-dhcp4:2.5.2
```

Kea DHCP v6 with all volumes and ports:
```shell
docker run --volume=./config/kea:/etc/kea  \
                --volume=./:/var/lib/kea  \
                -p [2008:db8::1]:547:547/tcp \
                -p [2008:db8::1]:547:547/udp \
                -p [2008:db8::1]:8000:8000 \
                -p [2008:db8::1]:8001:8001 \
